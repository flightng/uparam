...
static uint16_t uparam_readall() {
    ...
    for (int i = 0; i < header.cnt.u32; i++) {
        ...
        LOG_D("read param address 0x%X, size:%d, crc:%X, calc:%X", pa_this.address, pa_this.size, check,
              temp[pa_this.size]);
    ...
    for (int li = 0; li < param_index; li++) {
        ...
        for (int i = 0; i < ls[li].par_list_size; i++) {
            if ((ls[li].read_valid[i / 8] & (1 << i)) == 0) {
                //
                LOG_D("reset param [%-16s], address: 0x%X, size:%d", pa_list_t[i].name, pa_list_t[i].address,
                      pa_list_t[i].size);
                if (pa_list_t[i].default_fun != RT_NULL) {
                    pa_list_t[i].default_fun(pa_list_t[i].address, pa_list_t[i].size);
                } else {
                    LOG_E("reset error [%-16s], address: 0x%X, default_fun is null", pa_list_t[i].name,
                          pa_list_t[i].address);
                }
            }
        }
...
static void print_element(param_p *pa, uint32_t index, uint32_t offset) {
    ...
    if (pa_list->type[0] == 'f') {
        ...
        memcpy(value, (uint8_t *)pa->address, pa->size);
        if (pa->size == 1) {
            len = sprintf(buff, "Integer %d\r\n", *(int8_t *)(value));
        } else if (pa->size == 2) {
            len = sprintf(buff, "Integer %d\r\n", *(int16_t *)(value));
        } else if (pa->size == 4) {
            len = sprintf(buff, "Integer %d\r\n", *(int32_t *)(value));
        } else {
            len = sprintf(buff, "Integer Size Error\r\n");
        }
    } else if (pa_list->type[0] == 'u') {
        memcpy(value, (uint8_t *)pa->address, pa->size);
        if (pa->size == 1) {
            len = sprintf(buff, "Uintger %d\r\n", *(uint8_t *)(value));
        } else if (pa->size == 2) {
            len = sprintf(buff, "Uintger %d\r\n", *(uint16_t *)(value));
        } else if (pa->size == 4) {
            len = sprintf(buff, "Uintger %d\r\n", *(uint32_t *)(value));
        } else {
            len = sprintf(buff, "Uintger Size Error\r\n");
        }
...
static void uparam_list() {
    ...
    const uint32_t batch_size = 10;  // 每批打印10个参数
    ...
    for (int li = 0; li < param_index; li++) {
        ...
        for (int i = 0; i < ls[li].par_list_size; i++) {
            ...
            // 每打印一批参数后稍作延时，让系统有时间处理输出
            if (print_count % batch_size == 0) {
                rt_thread_mdelay(10);  // 延时10ms让输出缓冲区有时间处理
            }
        }
    }
...
static void erase_all_param(void) {
    ...
    for (int li = 0; li < param_index; li++) {
        uint16_t bit_num =
            (ls[li].par_list_size % 8 == 0) ? (ls[li].par_list_size / 8) : (ls[li].par_list_size / 8 + 1);
